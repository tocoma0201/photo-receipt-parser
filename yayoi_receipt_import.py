...
def extract_amount_candidates(text: str) -> tuple[list[tuple[int, int]], list[int], list[int]]:
    ...
    def looks_like_card_number_line(line: str) -> bool:
        lower = line.lower()

        card_words = (
            "visa", "master", "jcb", "amex", "diners",
            "カード", "card", "カイイン", "会員",
            "enekey", "enekeyid", "モバイル enekey", "モバイルenekey",
            "ポイントカード", "カード番号", "有効期限",
        )
        if any(word in lower for word in card_words):
            return True

        # 伏字付きカード番号や長い連番を含む行
        if "xxxx" in lower or "********" in line:
            return True
    ...
    for i, line in enumerate(lines):
        ...
        for blk_line in block:
            if looks_like_id_line(blk_line):
                continue
            if looks_like_card_number_line(blk_line):
                continue
    ...
    for line in lines:
        ...
        if looks_like_id_line(line) or looks_like_date_line(line) or looks_like_tax_or_subtotal_line(line) or looks_like_percent_tax_line(line):
            continue
        if looks_like_card_number_line(line):
            continue
...
def make_import_row(seq: int, result: ReceiptResult) -> list[object]:
    return [
...
def process_image(image_path: Path) -> ReceiptResult | SkipResult:
    ...
    if amount < NORMAL_PRICE_MIN or amount > NORMAL_PRICE_MAX:
        if date_warning!="":
           date_warning = date_warning + f","
        date_warning = date_warning + "額が範囲外が範囲外です"
...
def write_import(results: list[ReceiptResult], output_dir: Path, encoding: str) -> Path:
    ...
    with output_path.open("w", encoding=encoding, newline="") as f:
        ...
        for seq, r in enumerate(results, start=1):
            row = [safe_text_for_encoding(str(v), encoding) if isinstance(v, str) else v for v in make_import_row(seq, r)]
            writer.writerow(row)
    return output_path
...
def main() -> int:
    ...
    write_import(deduped, output_dir, encoding)
